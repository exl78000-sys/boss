...
async function claimUnboundPlayerRecords(player, showToast=false){
  ...
  // 舊資料相容：如果舊報名資料曾被其他帳號 ownerUid 綁定，也不允許搶綁。
  const legacyOtherOwner=records.some(s=>s.ownerUid && !s.unlinkedAt && s.ownerUid!==currentUser.uid);
  if(legacyOtherOwner){
    if(showToast)toast('此角色曾由其他 Google 帳號建立，不能重新綁定');
    return 0;
  }
...
function renderClaimBox(){
  ...
  }else if(legacyOtherOwner){
    E.claimStatus.textContent='此角色曾由其他 Google 帳號建立，不能重新綁定。';
    E.claimCharacterBtn.disabled=true;
